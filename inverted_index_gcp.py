...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        # Track the total number of tokens in each document
    ...
    def add_doc(self, doc_id, tokens):
        ...
        self.document_lengths[doc_id] = sum(w2cnt.values())
    ...
    def read_a_posting_list_tfidf(self, w):
        posting_list = self.read_a_posting_list(w)
        N = len(self.document_lengths)  # Total number of documents
        doc_ids_set = set()
        token_doc_tfidf_dict = {}
        ...
        for doc_id, tf in posting_list:
            ...
            idf = log(N / self.df[w])
            # Calculate TF-IDF
            tfidf = (tf / self.document_lengths[doc_id]) * idf
            token_doc_tfidf_dict[(w, doc_id)] = tfidf

        return doc_ids_set, token_doc_tfidf_dict
